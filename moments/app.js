...
function bindEvent() {
    // TODO: 完成页面交互功能事件绑定
    var $checkimg = $('.check-image');
    var $image = $checkimg.find('img');
    ...
    $('.moments-list').on('click', '.do-like', function() {
        var $eleitem = $(this).parents('.moments-item');
        var index = $eleitem.attr('data-index');
        ...
        $eleitem.find('.reply-like').remove();
        $eleitem.find('.reply-zone').prepend(htmlText);
    });

    //评论发送框
    $page.on('click', '.do-comment', function(event) {
        $('.commenter').remove();
        $(this).parents('.moments-item').append('<div class="commenter"><input class="commenter-input" text="text" placeholder="评论"><button class="send-btn" disabled="disabled">发送</button></div>');
    });
    ...
    $page.on('click', '.send-btn', function() {
        ...
        data[index].reply.comments.push(newmsg);
        var commentsHtml = commentsHtmlTpl(data[index].reply.comments);
        element.find('.reply-comment').remove();
        element.find('.reply-zone').append(commentsHtml);
        $('.commenter').remove();
    });
}
